 ...
 <xsl:variable name="scname">
  ...
  <xsl:template match="*" mode="detect">
    <xsl:choose>
      ...
      <xsl:when test="count(./child::*) = 0">
        <xsl:text>"</xsl:text><xsl:value-of select="name()"/>" : "<xsl:call-template name="escapeQuote"/><xsl:text>"</xsl:text>
        <xsl:if test="count(following-sibling::*) &gt; 0">, </xsl:if>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="*" mode="obj-content">
    ...
    <xsl:if test="count(child::*) = 0 and text() and @*">
      <xsl:text>"text" : "</xsl:text><xsl:call-template name="escapeQuote"/><xsl:text>"</xsl:text>
    </xsl:if>
    <xsl:text>}</xsl:text>
